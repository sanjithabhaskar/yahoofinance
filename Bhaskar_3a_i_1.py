...
# Save data to JSON
with open('Bhaskar_stock_holder_data.json', 'w', encoding='utf-8') as f:
    json.dump(stock_data, f, indent=4)

# Save data to CSV
CSV_FILE_PATH = 'Bhaskar_stock_holder_data.csv'
...
# Save data to Excel
EXCEL_FILE_PATH = 'Bhaskar_stock_holder_data.xlsx'
df = pd.DataFrame(list(stock_data.items()), columns=['Metric', 'Value'])
df.to_excel(EXCEL_FILE_PATH, index=False)

print('\nDone! Data saved to JSON, CSV, and Excel files.')
